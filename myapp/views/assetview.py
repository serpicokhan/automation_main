...
# @permission_required('myapp.view_asset')
def list_asset(request,id=None):
    books=[]
    books =filterUser(request)
    print(123)
    wos,page=AssetUtility.doPagingWithPage(request,books)
    return render(request, 'myapp/asset/assetList.html', {'asset': wos,'section':'list_asset','page':page})
...
def list_asset_location(request):
    ...
    wos,page=AssetUtility.doPagingWithPage(request,books)
    return render(request, 'myapp/asset/assettreeList.html', {'asset': wos,'section':'list_asset_location','page':page})
    # books=Asset.objects.filter(assetTypes=1)
    # return render(request, 'myapp/asset/assetList.html', {'asset': books})
...
def list_asset_machine(request):
    ...
    wos,page=AssetUtility.doPagingWithPage(request,books)
    return render(request, 'myapp/asset/assetList.html', {'asset': wos,'section':'list_asset_machine','page':page})
...
def list_asset_tool(request):
    ...
    wos,page=AssetUtility.doPagingWithPage(request,books)
    return render(request, 'myapp/asset/assetList.html', {'asset': wos,'section':'list_asset_tool','page':page})
...
def save_asset_form(request, form, template_name,id=None,page=None):
    ...
    if (request.method == 'POST'):
        if form.is_valid():
            ...
            wos,page=AssetUtility.doPagingWithPage(request,books)
            data['html_asset_list'] = render_to_string('myapp/asset/partialAssetList.html', {
                'asset': wos,
...
def asset_delete(request, id):
    ...
    if (request.method == 'POST'):
        ...
        context = {'asset': comp1}
        data['html_asset_form'] = render_to_string('myapp/asset/partialAssetDelete.html',
            context,
            request=request,
        )
    return JsonResponse(data)
...
def asset_create_location(request):
    if (request.method == 'POST'):
        ...
        return save_asset_form(request, form, 'myapp/asset/partialAssetLocationCreate.html')
    else:
        ...
        return save_asset_form(request, form, 'myapp/asset/partialAssetLocationCreate.html',assetInstance.id)

def asset_create_machine(request):
    if (request.method == 'POST'):
        form = AssetForm(request.POST)
        return save_asset_form(request, form, 'myapp/asset/partialAssetMachineCreate.html')
    else:
        assetInstance=Asset.objects.create(assetTypes=2)
        form = AssetForm({'assetHasPartOf':'False','assetStatus':'True','assetTypes':'2'})
        return save_asset_form(request, form, 'myapp/asset/partialAssetMachineCreate.html',assetInstance.id)

def asset_create_tool(request):
    if (request.method == 'POST'):
        form = AssetForm(request.POST)
        return save_asset_form(request, form, 'myapp/asset/partialAssetToolCreate.html')
    else:
        assetInstance=Asset.objects.create(assetTypes=3)
        form = AssetForm({'assetHasPartOf':'False','assetStatus':'True','assetTypes':'3'})
        return save_asset_form(request, form, 'myapp/asset/partialAssetToolCreate.html',assetInstance.id)
...
def asset_update(request, id):
    ...
    if(company.assetTypes==1):
        template="myapp/asset/partialAssetLocationUpdate.html"
    elif(company.assetTypes==2):
        template="myapp/asset/partialAssetMachineUpdate.html"
    else:
        template="myapp/asset/partialAssetToolUpdate.html"
...
def asset_type_selector(request,ids=None):
    ...
    #     data["form_asset_selector"]=render_to_string('myapp/asset/assetType.html',
    #         context,
    #         request=request)
    #
    #     return JsonResponse(data)
...
def get_assetCategory(request):
    ...
    m=m.replace('"',"'")
    data["modalassetcat"]=render_to_string('myapp/asset/assetcategoryselector.html',{'cat':m,'perms': PermWrapper(request.user)})
    return JsonResponse(data)
def get_assetCategoryMain(request,ids):
    ...
    if (request.method == 'POST'):
        ...
        m=m.replace('"',"'")
        context={'cat':m,'perms': PermWrapper(request.user),'ids':ids}
        data["modalassetcat"]=render_to_string('myapp/asset/assetcategoryselectorMain.html',
        context,
        request=request)
    return JsonResponse(data)
def duplicate_asset(request,id):
    ...
    if (request.method == 'POST'):
        ...
        data["modalassetcat"]=render_to_string('myapp/asset/assetDuplicateForm.html',
        context,
        request=request)
    return JsonResponse(data)
def get_location_by_category(request):
    ...
    m=m.replace('"',"'")
    data["modalassetcat"]=render_to_string('myapp/asset/locationcategoryselector.html',{'cat':m,'perms': PermWrapper(request.user)})
    return JsonResponse(data)
#######################Search By tags#####################
def asset_search(request,kvm):
    ...
    data['html_asset_search_tag_list'] = render_to_string('myapp/asset/partialAssetList.html', {
                   'asset': wos,'page':page                      ,'perms': PermWrapper(request.user) })
    data['html_asset_paginator'] = render_to_string('myapp/asset/partialAssetPagination.html', {
...
def asset_offline_status(request,id):
    ...
    data['html_assetOffline_recent']=render_to_string('myapp/asset/lastbreak.html', {
...
def list_asset_dash(request):
    ...
    assetloc=Asset.objects.filter(assetIsLocatedAt__isnull=True,assetTypes=1)
        ...
        # x1.push(x2)
        # x2=[]
    return render(request, 'myapp/asset/dash2.html', {'assets':final_list,'test':a_zip,'locs':assetloc})
...
def js_list_asset_dash(request,locId):
    ...
    data["html_asset_dash_list"]=render_to_string('myapp/asset/partialdash2.html',
        context,
        request=request)
    data["form_is_valid"]=True
    return JsonResponse(data)
...
def show_asset_types(request,ids):
    ...
    data["html_asset_type"]=render_to_string('myapp/asset/assetType_modal.html',
        context,
        request=request)
    return JsonResponse(data)
...
def show_Asset_status(request,id):
    ...
    data['html_asset_paginator'] = render_to_string('myapp/asset/partialAssetPagination2.html', {
...
def show_asset_tree(request,id):
    ...
    data=dict()
    data['form_is_valid']=True
    data['result']=render_to_string('myapp/asset/partialAssetTree.html', {
                      'assets': a})
    return JsonResponse(data)
@csrf_exempt
def js_list_assetWo(request,woId):
    ...
    data['html_assetWo_list']= render_to_string('myapp/asset_wo/partialAssetWoList.html', {
        'assetwos': books
    })
    data['form_is_valid']=True
    return JsonResponse(data)
def js_list_assetSWo(request,woId):
    ...
    data['html_assetSWo_list']= render_to_string('myapp/asset_swo/partialAssetWoList.html', {
        'assetwos': books
    })
    data['form_is_valid']=True
    return JsonResponse(data)
...
def js_list_assetCloseWo(request,woId):
    ...
    data['html_assetCloseWo_list']= render_to_string('myapp/asset_close_wo/partialAssetWoList.html', {
        'assetwos': books
    })
    data['form_is_valid']=True
    return JsonResponse(data)
...
def js_list_assetConsumedPart(request,woId):
    ...
    data['html_assetConsumedPart_list']= render_to_string('myapp/asset_consumed_part/partialAssetWoList.html', {
        'assetwos': books
    })
    data['form_is_valid']=True
    return JsonResponse(data)
...
def create_rowasset(request,wo):
    ...
    data['html_assetAsset_list']= render_to_string('myapp/asset/asset/partialAssetRow.html', {
        'assetwos': books,
        'perms': PermWrapper(request.user)
    })
    data['form_is_valid']=True
...
def assetAsset_craete(request):
    ...
    if (request.method == 'POST'):
        ...
        form = None
        data=dict()
        data['html_assetAsset_form']= render_to_string('myapp/asset_asset/partialAssetAssetCreate.html', {
...
def bulk_delete_asset(request,ids):
    ...
    data['html_asset_paginator'] = render_to_string('myapp/asset/partialAssetPagination_none.html', {
                      'asset': wos})
    data['form_is_valid'] = True
    # print('done')
    return JsonResponse(data)
...
def create_woasset(request):
    ...
    if (request.method == 'POST'):
        ...
        if form.is_valid():
           ...
           form = AssetForm()
           context={'form':form}
           data2['html_asset_form'] = render_to_string('myapp/asset/woasset/woasset_create.html',
               context,
               request=request,
           )
           return JsonResponse(data2)
...
def assetloadinfo(request):
    ...
    if(makan):
        ...
        data["html_assets_dynamics"]=render_to_string('myapp/maintenance/partialWOAssetDynamics.html',
            {'assets':assets})
        data["form_is_valid"]=True
    return JsonResponse(data)
